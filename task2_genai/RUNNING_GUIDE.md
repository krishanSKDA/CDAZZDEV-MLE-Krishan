...
## 1. Prerequisites
- Gemini API key (same as Tasks 1 & 3).
...
- This calls Gemini ~150 times (one per training example) -- expect a few
...
| Gemini API rate-limit errors during dataset generation | Request-per-minute limit | Add a short `time.sleep(1)` between calls in `generate_dataset`, or reduce `n` |
